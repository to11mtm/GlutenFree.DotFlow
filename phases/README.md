...
- [Phase3-3-WorkflowDesigner.md](Phase3-3-WorkflowDesigner.md) ⏳ — the browser-based visual designer (master plan + three breakouts: [3.3.a Foundation](Phase3-3a-DesignerFoundation.md) · [3.3.b Editing](Phase3-3b-DesignerEditing.md) · [3.3.c Runtime](Phase3-3c-DesignerRuntime.md)). **Blazor WebAssembly MVP** (user decision) on the existing `Workflow.UI`/`Workflow.UI.Client` skeleton, with framework-free C# state services + wire-DTO mirrors keeping a **React+TS port additive** (3.3.P7). Custom SVG/HTML canvas, schema-driven properties panel (`PropertyEditorType`) with **lazy-loaded Monaco** code editors, command-pattern undo/redo, two-stage save validation (client + new `POST /workflows/validate` over the existing validator — the one API addition), lightweight minimap, live run overlay via the 3.2 hub. ASCII mockups S1–S4 included. Weeks 27-30; Q1–Q7 resolved ✅~ 🎨
