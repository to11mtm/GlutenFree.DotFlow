...
- [Phase5-1-StreamingDataPlane.md](Phase5-1-StreamingDataPlane.md) 🔄 — **streaming regions**: opt-in stream-typed ports + Akka.Streams-backed region execution for bounded-memory large-dataset ETL. Design fully resolved in [06-streaming-data-plane-design.md](../new-feature-design/snaplogic-analysis/06-streaming-data-plane-design.md); **5.1.0–5.1.3 COMPLETE** (core contracts · stream bridges + bounded-accumulator guard · designer UX with diamond ports/region halos/drag-time refusal · region detection + `StreamRegionRunner` + `WorkflowExecutor` wiring, ~90 tests). Next: 5.1.4 module coverage + `maxWorkers`; Q6 (does `SelectAsync` obsolete the resequencer?) open~ 🌊
