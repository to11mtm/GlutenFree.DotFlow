...
public sealed class SqliteExecutionHistoryRepository : IExecutionHistoryRepository
{
	...
	public async Task RecordNodeExecutionAsync(NodeExecutionRecord nodeRecord, CancellationToken ct = default)
	{
		...
		if (existing is not null)
		{
			await db.ExecutionNodes
				...
				// Phase 2.2.3-followup: persist loop/subgraph/metadata fields on update too~ 🗂️
				.Set(n => n.LoopId, nodeRecord.LoopId)
				.Set(n => n.LoopIteration, nodeRecord.LoopIteration)
				.Set(n => n.SubGraphId, ExtractSubGraphId(nodeRecord))
				.Set(n => n.MetadataJson, SerializeMetadata(nodeRecord))
				.UpdateAsync(token: ct)
				.ConfigureAwait(false);
		}
		else
		{
			var entity = new ExecutionNodeEntity
			{
				...
				DurationMs = nodeRecord.Duration == default
					? null
					: (long)nodeRecord.Duration.TotalMilliseconds,
				// Phase 2.2.3-followup: persist loop/subgraph/metadata fields~ 🗂️
				LoopId = nodeRecord.LoopId,
				LoopIteration = nodeRecord.LoopIteration,
				SubGraphId = ExtractSubGraphId(nodeRecord),
				MetadataJson = SerializeMetadata(nodeRecord),
			};
	...
	private static NodeExecutionRecord MapToNodeRecord(ExecutionNodeEntity entity)
	{
		...
		// Phase 2.2.3-followup: restore Metadata from the two sources:
		// (a) the SubGraphId column (always keyed "subGraphId" in Metadata when present)
		// (b) the MetadataJson column (arbitrary extra keys stored as JSON dict)
		// Merge them so consumers see a single unified Metadata dictionary~ 🗂️
	...
	/// <summary>
	/// Extracts the <c>subGraphId</c> value from the record's <c>Metadata</c> dictionary.
	/// Returns <see langword="null"/> if <c>Metadata</c> is null or doesn't contain the key~ 🌿.
	/// CopilotNote: SubGraphId is stored as a first-class column for queryability, but also
	/// lives in <c>Metadata["subGraphId"]</c> on the model to keep the read path uniform~ 💖.
	/// </summary>
	private static string? ExtractSubGraphId(NodeExecutionRecord record)
		=> record.Metadata?.TryGetValue("subGraphId", out var id) == true
			? id?.ToString()
			: null;

	/// <summary>
	/// Serializes the record's <c>Metadata</c> dictionary to JSON, excluding the <c>subGraphId</c>
	/// key (which is stored in its own dedicated column)~ 🗂️.
	/// Returns <see langword="null"/> when no remaining keys exist after exclusion.
	/// </summary>
	private static string? SerializeMetadata(NodeExecutionRecord record)
	{
		if (record.Metadata is null || record.Metadata.Count == 0)
		{
			return null;
		}

		// Exclude subGraphId — it lives in its own column
		var extra = record.Metadata
			.Where(kv => kv.Key != "subGraphId")
			.ToDictionary(kv => kv.Key, kv => kv.Value);

		return extra.Count > 0 ? JsonSerializer.Serialize(extra, JsonOptions) : null;
	}

	/// <summary>
	/// Reconstructs the unified <c>Metadata</c> dictionary from the entity's dedicated columns
	/// and the JSON metadata blob~ 🗂️.
	/// </summary>
	private static IReadOnlyDictionary<string, object?>? BuildMetadata(ExecutionNodeEntity entity)
	{
		Dictionary<string, object?>? result = null;

        // Restore subGraphId from dedicated column~ 🌿
        if (entity.SubGraphId is not null)
        {
            // CopilotNote: result is guaranteed null here (first possible initialisation path),
            // so assign directly rather than using ??= to avoid the "dead conditional" analyser warning.
            result = new Dictionary<string, object?> { ["subGraphId"] = entity.SubGraphId };
        }

		// Merge any extra keys from the JSON metadata blob
		if (entity.MetadataJson is not null)
		{
			var extra = JsonSerializer.Deserialize<Dictionary<string, object?>>(entity.MetadataJson, JsonOptions);
			if (extra is { Count: > 0 })
			{
