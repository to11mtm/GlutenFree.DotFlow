// <copyright file="WorkflowEntity.cs" company="GlutenFree">
// Copyright (c) GlutenFree. All rights reserved.
// </copyright>

namespace Workflow.Persistence.Sqlite.Data.Entities;

using LinqToDB.Mapping;

/// <summary>
/// 📋 Linq2Db entity mapping for the <c>workflows</c> table~ ✨💖
/// </summary>
[Table("workflows")]
public class WorkflowEntity
{
    /// <summary>Gets or sets the workflow UUID as a string~ 🆔.</summary>
    [PrimaryKey]
    [Column("id")]
    public string Id { get; set; } = string.Empty;

    /// <summary>Gets or sets the workflow name~ 🏷️.</summary>
    [Column("name")]
    [NotNull]
    public string Name { get; set; } = string.Empty;

    /// <summary>Gets or sets the optional description~ 📝.</summary>
    [Column("description")]
    [Nullable]
    public string? Description { get; set; }

    /// <summary>Gets or sets the serialized <c>WorkflowDefinition</c> JSON~ 📦.</summary>
    [Column("definition")]
    [NotNull]
    public string Definition { get; set; } = string.Empty;

    /// <summary>Gets or sets the semantic version string~ 📊.</summary>
    [Column("version")]
    [NotNull]
    public string Version { get; set; } = string.Empty;

    /// <summary>Gets or sets 1 if active, 0 if soft-deleted~ 🔘.</summary>
    [Column("is_active")]
    [NotNull]
    public int IsActive { get; set; } = 1;

    /// <summary>Gets or sets the ISO-8601 creation timestamp~ 📅.</summary>
    [Column("created_at")]
    [NotNull]
    public string CreatedAt { get; set; } = string.Empty;

    /// <summary>Gets or sets the ISO-8601 last-updated timestamp~ 🔄.</summary>
    [Column("updated_at")]
    [NotNull]
    public string UpdatedAt { get; set; } = string.Empty;

    /// <summary>Gets or sets the comma-joined tag list (nullable)~ 🏷️.</summary>
    [Column("tags")]
    [Nullable]
    public string? Tags { get; set; }

    /// <summary>Gets or sets the metadata JSON blob (nullable)~ 🗂️.</summary>
    [Column("metadata")]
    [Nullable]
    public string? Metadata { get; set; }
}

