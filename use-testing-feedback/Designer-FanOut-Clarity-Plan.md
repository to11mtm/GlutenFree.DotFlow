# Designer FanOut — Partitioned Legs & Clarity — Plan

> 📋 Response to user feedback (2026-08-04,
> [Designer-FanOut-Clarity.md](Designer-FanOut-Clarity.md)): *"a way to have items fan out based on
> their items… Foo on one output and Bar and Baz on a separate leg"*, and *"they are uncertain how
> it works in current state overall — better clarity around the existing case"*.
>
> **Revision 1 — proposal.** Feedback 2 (clarity) is well-understood and can proceed. Feedback 1
> (partitioned legs) has a genuine design fork — **Q1–Q3 below need answers before P-items start.**
...
| # | Feedback | Reading |
| --- | --- | --- |
| 1 | "Fan out based on their items — separate outputs for Foo, Bar, Baz" | Route each item of a collection to a leg chosen by **what the item is** |
| 2 | "…or Foo on one output and Bar and Baz on a separate leg" | Legs are **user-defined groups**, not necessarily one-per-kind |
...
| `builtin.parallel` | *(none — pure trigger)* | named, static | *(all fire)* | no data routing at all |
| `builtin.fanout` | a collection | **one** (`branch`) | *(no choice — every item)* | every item, same leg, concurrently |
| **the ask** | a collection | named, user-defined | **what each item is** | each item goes to the leg its group says |

So the ask is genuinely new: **Switch's routing table applied per-item to FanOut's collection** — a
*partition*. `Foo → leg A; Bar, Baz → leg B` is exactly a Switch `cases` table, except evaluated
...
| --- | --- | --- | --- | --- |
| **Clarity (feedback 2 — not blocked)** | | | | |
| K1 | FanOut `branch` sub-graph gets a visual region on the canvas (like loop bodies have) | UX | S | ⬜ |
| K2 | Port tooltips on the canvas — surface each port's schema description on hover | UX | S | ⬜ |
| K3 | FanOut port/property descriptions rewritten for beginners (say "each item", name the `item`/`index` payload) | Docs/UX | S | ⬜ |
| K4 | Docs: a worked "what actually happens" walkthrough for fanout (items → N branches → fanin) | Docs | S | ⬜ |
| K5 | Designer warning: FanOut `branch` wired to nothing (runs N times into the void) | UX | S | ⬜ |
| **Partitioned legs (feedback 1 — decided, Q1 = both modules)** | | | | |
| P1 | `builtin.partition` module — split a collection into named legs by per-item routing rules | Feature | M | ⬜ |
| P1b | `builtin.split` module — object-property split: one output port per named property (Q1 = both) | Feature | M | ⬜ |
| P2 | Designer support: dynamic output ports from the routing table (Switch already sets the precedent) | UX | M | ⬜ |
| P3 | Region + clarity affordances for P1 (reuse K1/K2 machinery) | UX | S | ⬜ |
| P4 | Tests | Tests | M | ⬜ |
| P5 | Docs | Docs | S | ⬜ |
...
### F1 — "Fan out by item" is a partition, and nothing does it today
...
- `SwitchModule` routes **one value** to one of many named ports via its `cases` table
  ...
  users described — but for a single value, not a collection's items.
...
- `builtin.parallel` has named legs but routes **no data** — its branch ports are pure triggers.

### F2 — Two different reads of "the input has Foo and Bar and Baz" ⚠️

The feedback supports two readings, and they are **different modules**:

- **(a) Collection partition** — input is a *list* of items of kinds Foo/Bar/Baz; each item goes to
  the leg its kind maps to. Legs receive **sub-collections**.
- **(b) Object split** — input is an *object* with properties `Foo`, `Bar`, `Baz`; each property
...
The "Bar and Baz on a separate leg" phrasing fits both. **This is Q1** — the single most important
question, because (a) and (b) share almost no implementation.

### F3 — Loops get a visual body region; fanout doesn't. That's most of feedback 2

`StructuralRegions` draws a labelled box around the downstream closure of `loopBody` / `try` /
`catch` / `finally` / `transactionBody` — but `NodePorts.StructuralPortNames` does **not** include
...
fix is adding `branch` to the structural-region machinery with a label like *"🌟 per item
(parallel)"*.
...
region computation can consult the source node's module id (it has the node — likely trivial).

### F4 — The `item`/`index` payload is invisible until runtime

Inside the branch sub-graph, each run receives `item` + `index` inputs — but nothing in the
...
### F5 — FanOut's `failFast` metadata contradicts itself 🐛 (drive-by)
...
metadata mismatch as part of K3 (decide which default is intended — the sibling `parallel` module
defaults `true`).

### F6 — Related plan overlap
...
## Proposed design (P-items) 🎨 — pending Q1

Assuming Q1 = collection partition (recommended; see Q1):

**`builtin.partition`** — "Split items into legs." One `items` input (same coercion as
...
```jsonc
{
  ...
  "properties": {
    ...
    "defaultPort": "unmatched"     // optional; omitted → unmatched items dropped? (Q3)
  }
}
```

- Each output port emits **the sub-collection** of items that matched it (empty list if none —
  ports always fire, so downstream isn't silently skipped; consistent with D2 below).
- Two rules mapping to the same port (Bar *and* Baz → `others`) is the "separate leg" ask — no
  special grouping syntax needed, the table already expresses it.
- **Sequential, not parallel** — partition just *sorts items into buckets*. Users who want
...
- Dynamic output ports exactly like Switch (empty declared outputs → port-name validation skipped);
...
| **D2 Empty legs still emit `[]`** | A leg that silently never fires is the F2-of-the-last-plan trap all over again. Empty list ≠ no signal. |
...
- [x] **Q1 — Which reading of "the input has Foo and Bar and Baz" (F2)?** ✅ **Both, as two
      separate modules**: `builtin.partition` (collection → legs by rules) and `builtin.split`
      (object property → its own port). Each does one explainable thing (D1 spirit).
- [x] **Q2 — How do items declare their kind?** ✅ `matchOn` property path with plain string
      equality (default: the item itself). Can grow into predicates later.
- [x] **Q3 — What happens to unmatched items?** ✅ With `defaultPort` set they go there; without
      it, **fail the run** listing the unmatched values. Silent dropping is invisible data loss.
- [x] **Q4 — Should `builtin.parallel`'s branch ports also get regions (K1 scope)?** ✅ Yes, if
      cheap — same mechanism (noting its ports are unwireable until the other plan's J11).
- [x] **Q5 — FanOut `failFast` default (F5)?** ✅ `true` is intended — fix the property metadata
...
### K1 — FanOut branch region 🌟

- [x] K1.1 Add fanout's `branch` to the structural-region derivation, labelled
      *"🌟 per item (parallel)"* — kind `fanout` for CSS.
- [x] K1.2 Guard against port-name collisions with non-fanout modules (F3 caveat) — region only
...
### K3 — FanOut speaks human 🗣️

- [x] K3.1 Rewrite FanOut's module/port/property descriptions: `branch` = *"runs once per item —
...
- [x] K4.1 Add a "What actually happens" subsection under `builtin.fanout` in
      [`docs/advanced-flow-control.md`](../docs/advanced-flow-control.md): items in → branch runs
      per item with `item`/`index` → results/fanin out, with a small diagram.
- [x] K4.2 Cross-link switch vs fanout vs parallel vs (future) partition — the table from *The gap
      in one table* belongs in the docs.

### K5 — Unwired branch warning 🧭

- [x] K5.1 `GraphValidator`: FanOut node whose `branch` port has no outgoing connection → warning
      *"Fan Out's branch isn't connected — each item will run an empty sub-graph."*
- [x] K5.2 Tests.

### P1–P5 — Partitioned legs & object split (Q1 = both)
...
- [x] P1b `builtin.split` — input `value` (object); property `keys` (JSON array of property
      names, each becomes an output port emitting that property's value); optional `restPort`
      (remaining properties as one object). Missing key → `null` output (objects legitimately have
...
- [x] P2 Designer derives output ports from the `rules` property (partition) and the
      `keys`/`restPort` properties (split) — extend the `NodePorts` dynamic machinery; keep it
