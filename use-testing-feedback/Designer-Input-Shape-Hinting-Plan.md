...
## Open questions ❓ — Q1–Q3 decided 2026-08-04 ✅; Q4 expanded below

- [x] **Q1 — Scope?** ✅ Picker + panel section (all of design §1–3).
- [x] **Q2 — Insert or copy from the panel section?** ✅ Insert into the focused property editor,
      falling back to clipboard when none is focused.
- [x] **Q3 — Runtime samples?** ✅ Deferred to a follow-up round together with the Split-preview
      plan's Q3 (execution-history plumbing benefits both at once).
- [x] **Q4 — FanIn?** ✅ In scope — design proposed below (§FanIn hinting), with its own
      questions (Q5–Q7).

## FanIn hinting — proposed design 🪄 (from Q4)
...
is connection *declaration* order — deterministic (the engine iterates
`Connections.Where(target == fanin)` in saved order; FanIn's own remarks confirm it) but shown
nowhere. *(Note: `advanced-flow-control.md` claims Concat is "branch-completion order" — that's
...
```
┌─ Incoming branches (order matters for merge/first/last) ─┐
│ 1. http-1 · body                                         │
│ 2. http-2 · body                                         │
│ 3. script-1 · result                                     │
└──────────────────────────────────────────────────────────┘
...
| `named` | the **computed key list** — source port names, falling back to `nodeId.port` on collision, mirroring the engine's `NamedBranches` naming rule exactly |
| `first` / `last` | "result = branch 1's payload" / "…branch N's payload" |

**3. Downstream tokens**: nodes after a FanIn get picker entries derived from the same
computation — `{{fanin-1.result.<key>}}` for `named` (keys are computable), `{{fanin-1.result}}` +
`{{fanin-1.count}}` for the rest.

Like the Split preview, §2/§3 **mirror engine semantics client-side** (the Named collision rule,
...
arity/ports mode (J4), the sub-graph zero-branches bug (J2 — a runtime bug, not a hinting
concern; see Q7 for how the hint handles it honestly).

### FanIn questions ❓ — decided 2026-08-04 ✅

- [x] **Q5 — Scope confirmation:** ✅ All three parts (branch list, mode-aware shape, downstream
      tokens), client-side with drift guards.
- [x] **Q6 — Show branch order as editable?** ✅ **Reorder controls in this round** — up/down
...
- [x] **Q7 — FanIn inside a sub-graph (J2)?** ✅ **Both**: fix the engine bug (SubGraphExecutor
      doesn't populate `__incomingBranches__`/`__incomingBranchMeta__` — silent zero-branch
...
## Addendum — the `{{input}}` root (from the HTTP-input plan) 🔌
...
port is wired — it's the beginner-friendliest spelling ("the value coming in"), and for merged
...
| T2 | `VariableTokens.OptionsFor()` offers `{{input}}` first (D-I), merged sub-keys, type/description details (design §1–2) | UX | S | ⬜ |
| T3 | Token picker rows render detail text (type — description) | UX | S | ⬜ |
| T4 | "Incoming data" PropertiesPanel section (design §3, Q2 insert-with-clipboard-fallback) | UX | M | ⬜ |
| T5 | Tests (helper unit tests; picker options; panel rendering; insert/copy behavior) | Tests | M | ⬜ |
| T6 | Docs: composition guide snippet — "how to reference upstream data", incl. merged-mode addressing; fix the Concat ordering claim in `advanced-flow-control.md` | Docs | S | ⬜ |
| T7 | FanIn hinting per §FanIn design (Q5): branch list, mode-aware shape w/ drift guards, downstream tokens | UX | M | ⬜ |
| T8 | Branch reorder controls on FanIn's branch rows — undoable connection-reorder command (Q6) | UX | M | ⬜ |
| T9 | 🐞 Fix J2: `SubGraphExecutor.GatherNodeInputs` populates `__incomingBranches__`/`__incomingBranchMeta__` like `WorkflowExecutor` (Q7); regression tests | Core | S | ⬜ |
